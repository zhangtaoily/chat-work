...
**在未来组织中的潜在角色——「集团大脑」**：各业务系统是集团的「器官」（各司其职），本系统的演进方向是成为协调各器官、连接人与组织的「大脑」：

1. **统一神经入口**：自然语言是唯一不需要培训的企业 UI，人向大脑提问/下指令，大脑翻译为对各业务系统的编排调用
2. **组织记忆**：对话记忆 + 知识库 + 审计流水沉淀为可继承的组织知识——人会离职，经验留下
3. **自动化执行臂**：定时周报、巡检、预警等自动化任务让大脑 7×24 值守，例行决策自动化，人聚焦例外决策
4. **治理内建**：权限护栏、审计、幂等与灰度机制，保证大脑的每个动作可追溯、可回滚、可控
...
| 阶段 | 目标 | 关键动作 | 验收标志 |
|------|------|----------|----------|
| **一：单机 mock 链路（已完成）** | 打通协议契约与交互流程 | 七域 MCP Server + agent-core 流水线 + 桌面端/小程序 | pytest 297 绿；HITL 确认卡闭环 |
...
| **四：集团大脑演进** | 从工具到组织智能 | 组织记忆沉淀（跨人会话知识入库）；自动化任务扩面（巡检/预警/周报）；跨域编排技能（如「订单→生产→发货」全链路）；小程序成为移动触点 | 例行决策自动化率提升；新员工零培训上手 |
